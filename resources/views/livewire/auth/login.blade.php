...
<head>
    ...
    <!-- plugin "forms" wajib disertakan, tanpa ini checkbox & input tidak ikut ter-restyle -->
        ...
        tailwind.config = {
            theme: {
                extend: {
                    ...
                    boxShadow: {
                        soft: '0 20px 45px -15px rgba(15, 31, 61, 0.25)',
                    },
                },
            },
        }
...
<div class="min-h-screen flex">
    ...
    <div class="hidden lg:flex w-1/2 relative bg-gradient-to-br from-navy-950 via-navy-900 to-navy-800 text-white flex-col justify-between p-12 overflow-hidden">
        ...
        {{-- glow ambient, memberi kedalaman tanpa jadi ramai --}}
        ...
        <div class="relative flex items-center gap-3">
            <div class="h-10 w-10 rounded-lg bg-accent flex items-center justify-center font-display font-bold shadow-soft">CT</div>
        ...
        <div class="relative max-w-md">
            ...
            <ul class="mt-8 space-y-3 text-sm text-slate-300">
                <li class="flex items-center gap-3">
                    <span class="h-5 w-5 rounded-full bg-sky-500/20 text-sky-400 flex items-center justify-center text-xs">✓</span>
                    Riwayat peminjaman tercatat rapi
                </li>
                <li class="flex items-center gap-3">
                    <span class="h-5 w-5 rounded-full bg-sky-500/20 text-sky-400 flex items-center justify-center text-xs">✓</span>
                    Ketersediaan peralatan real-time
                </li>
                <li class="flex items-center gap-3">
                    <span class="h-5 w-5 rounded-full bg-sky-500/20 text-sky-400 flex items-center justify-center text-xs">✓</span>
                    Akses aman, khusus Admin
                </li>
            </ul>
        </div>
    ...
    {{-- RIGHT — Login form --}}
    <div class="flex-1 flex items-center justify-center p-6 sm:p-12 bg-[#F5F7FA]">
        <div class="w-full max-w-sm bg-white sm:shadow-soft sm:border sm:border-slate-100 rounded-2xl sm:p-8" x-data="{ showPassword: false }">
            ...
            <form method="POST" action="{{ route('login') }}" class="space-y-5">
                ...
                <div>
                    ...
                    <input id="email" type="email" name="email" value="{{ old('email') }}" required autofocus autocomplete="username"
                           placeholder="nama@cakrawala.test"
                           class="w-full text-sm rounded-xl border-slate-200 focus:border-sky-500 focus:ring-sky-500 focus:ring-2 transition">
                ...
                <div>
                    ...
                    <div class="relative">
                        <input :type="showPassword ? 'text' : 'password'" id="password" name="password" required autocomplete="current-password"
                               placeholder="••••••••"
                               class="w-full text-sm rounded-xl border-slate-200 focus:border-sky-500 focus:ring-sky-500 focus:ring-2 transition pr-11">
                        <button type="button" @click="showPassword = !showPassword"
                                class="absolute inset-y-0 right-0 px-3 flex items-center text-slate-400 hover:text-slate-600 text-xs font-semibold">
                ...
                <button type="submit"
                        class="w-full bg-navy-900 hover:bg-navy-800 active:bg-navy-950 text-white text-sm font-semibold py-3 rounded-xl transition shadow-sm">
                    Masuk
                </button>
            </form>
